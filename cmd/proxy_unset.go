...
func init() {
	proxyUnsetCmd.Flags().StringSliceVar(&unsetTargets, "targets", []string{"all"}, "comma-separated targets (shell,session-env,git,npm,vscode,gnome,kde,dockerd,docker-config,lxd,snap,apt,all)")
